...
class ConfigManager:
    ...
    def _get_background_config(self):
        ...
        for group in self.app.background_groups:
            group_data = {
                ...
                'enabled': safe_group_get(group, 'enabled', False),
                ...
                'interval': safe_group_get(group, 'interval', '5'),
                'pause': safe_group_get(group, 'pause', '180'),
                'key': safe_group_get(group, 'key', ''),
                'delay_min': safe_group_get(group, 'delay_min', '100'),
                'delay_max': safe_group_get(group, 'delay_max', '200'),
                'click_enabled': safe_group_get(group, 'click_enabled', False),
                'click_offset': safe_group_get(group, 'click_offset', '0'),
                'alarm': safe_group_get(group, 'alarm', False)
            }
            
            group_type = group.get('type', 'ocr')
            if group_type == 'ocr':
                group_data.update({
                    'keywords': safe_group_get(group, 'keywords', ''),
                    'language': safe_group_get(group, 'language', '简体中文')
                })
            elif group_type == 'image':
                group_data.update({
                    'reference_image': group.get('reference_image', ''),
                    'threshold': safe_group_get(group, 'threshold', '80')
                })
            elif group_type == 'color':
                group_data.update({
                    'target_color': group.get('target_color'),
                    'tolerance': safe_group_get(group, 'tolerance', '10')
                })
        ...
        return {
            'target_window': {
                'title': getattr(self.app, 'bg_window_title_var', None)
            },
            'switch_delay': 0.05,
            'restore_delay': 0.02,
    ...
    def _get_home_checkboxes_config(self):
        """获取首页勾选框配置"""
        return {
            'ocr': safe_group_get(self.app.module_check_vars, 'ocr', False),
            'timed': safe_group_get(self.app.module_check_vars, 'timed', False),
            'number': safe_group_get(self.app.module_check_vars, 'number', False),
            'image': safe_group_get(self.app.module_check_vars, 'image', False),
            'script': safe_group_get(self.app.module_check_vars, 'script', False),
            'background': safe_group_get(self.app.module_check_vars, 'background', False)
        }
