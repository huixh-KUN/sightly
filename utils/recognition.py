...
class OCRRecognizer:
    """统一的OCR识别器（基于RapidOCR，支持中英文游戏文字）"""
    ...
    @staticmethod
    def recognize(image, keywords: str, language: str = "chi_sim", 
        ...
        try:
            ...
            text = OCRRecognizer.get_text(image, language)
            if not text:
                return (False, None)
...
class NumberRecognizer:
    ...
    @staticmethod
    def recognize(image, whitelist: str = "0123456789/",
                  confidence_threshold: float = 0.0) -> Optional[str]:
        """
        数字OCR识别
        ...
        Args:
            image: PIL.Image 图像
            whitelist: 允许的字符白名单
            confidence_threshold: 置信度阈值（0.0-1.0），低于此值的结果丢弃
